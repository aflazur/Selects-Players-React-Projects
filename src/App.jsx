...
function App() {
  ...
  const [availableBalance, setAvailableBalance] = useState(900000);
  const [purchasedPlayers, setPurchasedPlayer] = useState([])
    ...
    <>
      ...
      <div className='max-w-11/12 mx-auto flex justify-between items-center mt-12 mb-8'>
        ...
        <div className='font-bold'>
          ...
          <button onClick={() => setToggle(false)} className={`py-3 px-4 border-1 border-gray-400 rounded-r-2xl border-l-0 ${toggle === false ? "bg-[#E7FE19]" : ""}`}>Selected <span>({purchasedPlayers.length})</span></button>
        </div>

      </div>
      {
        toggle === true ? <Suspense fallback={<span className="loading loading-spinner loading-xl"></span>}>
          <AvailablePlayers purchasedPlayers={purchasedPlayers} setPurchasedPlayer={setPurchasedPlayer} availableBalance={availableBalance} setAvailableBalance={setAvailableBalance} playersPromise={playersPromise}></AvailablePlayers>
        </Suspense>
